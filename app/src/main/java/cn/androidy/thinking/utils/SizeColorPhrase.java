 ...
 * <p/>
 * <pre>
 * CharSequence chars = ColorPhrase.from(&quot;I'm&lt;Chinese&gt;,I love &lt;China&gt;&quot;).withSeparator(&quot;&lt;&gt;&quot;).innerColor(0xFFE6454A)
 * 		.outerColor(0xFF666666).format();
 * </pre>
 * <ul>
 * <li>Surround keys with curly braces; use two {{ to escape.</li>
 * <li>Fails fast on any mismatched keys.</li>
 * </ul>
 * The constructor parses the original pattern into a doubly-linked list of {@link Token}s. These tokens do not modify
 * the original pattern, thus preserving any spans.
 * <p/>
...
public class SizeColorPhrase {
    ...
    public static SizeColorPhrase from(Fragment f, int patternResourceId) {
     ...
     * @throws IllegalArgumentException if pattern contains any syntax errors.
     */
    public static SizeColorPhrase from(View v, int patternResourceId) {
     ...
     * @throws IllegalArgumentException if pattern contains any syntax errors.
     */
    public static SizeColorPhrase from(Context c, int patternResourceId) {
     ...
     * @throws IllegalArgumentException if pattern contains any syntax errors.
     */
    public static SizeColorPhrase from(Resources r, int patternResourceId) {
     ...
     * @throws IllegalArgumentException if pattern contains any syntax errors.
     */
    public static SizeColorPhrase from(CharSequence pattern) {
        return new SizeColorPhrase(pattern);
    }

    private SizeColorPhrase(CharSequence pattern) {
     ...
     * @return
     */
    public SizeColorPhrase withSeparator(String _separator) {
     ...
     * @return
     */
    public SizeColorPhrase outerColorSize(int _outerColor, float _outerSize) {
        this.outerColor = _outerColor;
        this.outerSize = _outerSize;
        return this;
    }

    public SizeColorPhrase outerColor(int _outerColor) {
        this.outerColor = _outerColor;
        return this;
    }

    public SizeColorPhrase outerSize(float _outerSize) {
        this.outerSize = _outerSize;
        return this;
    }
     ...
     * @return
     */
    public SizeColorPhrase innerColorSize(int _innerColor, float _innerSize) {
        this.innerColor = _innerColor;
        this.innerSize = _innerSize;
        return this;
    }

    public SizeColorPhrase innerColor(int _innerColor) {
        this.innerColor = _innerColor;
        return this;
    }

    public SizeColorPhrase innerSize(float _innerSize) {
        this.innerSize = _innerSize;
        return this;
    }
    ...
    private InnerToken inner(Token prev) {
        ...
        return new InnerToken(prev, key, innerColor, innerSize);
    }
    ...
    private OuterToken outer(Token prev) {
        ...
        return new OuterToken(prev, curCharIndex - startIndex, outerColor, outerSize);
    }
    ...
    /**
     * Advances the current character position without any error checking. Consuming beyond the end of the string can
     * only happen if this parser contains a bug.
     */
    ...
    /**
     * Returns the raw pattern without expanding keys; only useful for debugging. Does not pass through to
     * {@link #format()} because doing so would drop all spans.
     */
    @Override
    ...
    private static class OuterToken extends Token {
        ...
        OuterToken(Token prev, int textLength, int _color, float _outerSize) {
            super(prev);
        ...
        void expand(SpannableStringBuilder target) {
            ...
            if (outerSize > 0) {
                target.setSpan(new AbsoluteSizeSpan((int) outerSize), startPoint, endPoint,
    ...
    private static class InnerToken extends Token {
        ...
        InnerToken(Token prev, String _inner, int _color, float _innerSize) {
            super(prev);
        ...
        void expand(SpannableStringBuilder target) {
            ...
            if (innerSize > 0) {
                target.setSpan(new AbsoluteSizeSpan((int) innerSize), replaceFrom, replaceTo - 2,
                        Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
